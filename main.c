...
static inline int close_sockets(struct pollfd* begin, struct pollfd* end){
    ...
    if(cse){
        fprintf(stderr, "Failed to close %d client socket%s\n", cse, cse > 1 ? "s" : "");
        return ERR;
    }
    return OK;
}
...
static inline void finalize_robin(struct pollfd* polls, size_t client_count, size_t* sent_stamps){
...
int round_robin_mode(int signal_fd, int server_fd, size_t max_connections, int verbosity, int timeout, bool unidirectional){
    //TODO: find a better way to deduce the slice_count
    int timeout_slice_count = 10;
    timeout *= 100;
    size_t client_count = 0;
    struct pollfd* polls = malloc((2 + max_connections + 1) * sizeof(struct pollfd));
    size_t generation = 0;
    size_t idle_gens = 0;
        ...
        if(polls[1].revents & POLLIN){
            ...
            if(client_count == max_connections){
                ...
                if(biggest_diff >= timeout_slice_count){
                    if(verbosity > 1) puts("kicking out longest idling socket due to overflow");
                    ...
                    if(verbosity > 1) puts("rejecting new socket due to overflow");
            ...
            do{
                ...
                if(read_count <= 0){
                    ...
                    *sender = clients[client_count];
                    active_stamps[sender - clients] = active_stamps[client_count];
                }
                else{
                    action = true;
                    receiver = clients;
                    while(receiver != clients + client_count){
                        if(receiver != sender){
                            ssize_t write_count = write(receiver->fd, buffer, read_count);
                            if(interrupted) {
                                finalize_robin(polls, client_count, active_stamps);
                                return isErrounuousInterrupt();
                            }
...
int connection_mode(int signal_fd, int server_fd, size_t max_pending, int timeout, bool unidirectional, int verbosity){
    //TODO: allow n people in one closed down connection
    size_t client_count = 0;
    bool established = false;
    struct pollfd* polls = malloc((2 + max_pending + 1) * sizeof(struct pollfd));
    struct pollfd* clients = polls + 2;
    if(!polls){
        fputs("allocation failiure", stderr);
        return ERR;
    ...
    polls[1].fd = server_fd;
    while(!interrupted) {
        int event_count = poll(polls, client_count + 2, established ? timeout * 1000 : 0); 
        if(event_count == 0){
            if(established){
                if(verbosity > 0)puts("ending established connection due to timeout");
                established = false;
        ...
        if(polls[1].revents & POLLIN){
            ...
            if(established){
                ...
                if(client_count == max_pending){
                    if(verbosity > 1) puts("kicking out pending socket due to overflow");
                    close_socket(clients[0].fd);
                    memmove(clients, clients + 1, (client_count - 1) * sizeof(struct pollfd));
            ...
            while(event_count > 0 && client_count >= 2){
                ...
                if(read_count <= 0){
                    if(!established){
                        close_socket(sender->fd);
                        client_count--;
                        //we can't just swap with the last as we want to keep the order
                        memmove(
                            sender,
                            sender + 1,
                            (client_count - (sender - clients)) * sizeof(struct pollfd)
                        ); 
                    ...
                    if(write_count != read_count){
                        if(!established){
                            ...
                            memmove(
                                receiver,
                                receiver + 1,
                                (client_count - (receiver - clients)) * sizeof(struct pollfd)
                            ); 
...
int parseTimeout(const char* c, int* timeout){
    ...
    int handle_unit(int vals_entry, size_t seconds){
        ...
        if(val > INT_MAX / seconds){
            print_error_overflow(); return ERR;
        }
        seconds *= val;
    ...
    while(*a == '-'){
        ...
        do{
            ...
            if(*a == 'r'){
                ...
                *verbosity++;
                if(*verbosity == 4){
                    fprintf(stderr, "There are no more than 3 verbosity levels\n", *a);
                }
            }
            else if(*a == 't'){
                a++;
                ...
                char* val_name = (!round_robin) ? "max pending connections" : "max connections";
                a++;
                if(*a != '\0'){
                ...
                if(*max_connections < 2){
                    fprintf(stderr, "<%s> must be greater than 2 to allow for communication\n", val_name);
                    return ERR;
                }
                break;
            }
...
int main(int argc, char** argv){
    ...
    int verbosity;
    size_t somaxconn;
    if(parse_cmd_args(argc, argv, &port, &round_robin, &timeout, &max_connections, &unidirectional, &verbosity)){
        return EXIT_FAILURE;
    }
    ...
    int rc;
    if(round_robin){
        rc = round_robin_mode(signal_fd, server_fd, max_connections, verbosity, timeout, unidirectional);
    }
    else{
        rc = connection_mode(signal_fd, server_fd, max_connections, verbosity, timeout, unidirectional); 
    }
    if(close(signal_fd)){
